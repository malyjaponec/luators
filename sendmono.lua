...
local function Start()
	...
	-- pocatek kriticke sekce
        ...
        energy,Energy_Faze = Energy_Faze,0
    -- konec kriticke sekce
        
	-- sam si akumuluji hodnoty k odeslani ziskane z merice a mazu je jen po uspesnem predani
	SentEnergy_Faze = rtcmem.read32(1,1) -- vyctu si hodnotu z RTC memory 
	SentEnergy_Faze = SentEnergy_Faze + energy -- prictu aktualni citace za posledni periodu
	rtcmem.write32(1, SentEnergy_Faze)-- zapisu si hodnoty do RTC memory
	suma = suma + SentEnergy_Faze -- scitam si kontrolni soucet
	...
	Rdat[Rpref.."e"] = SentEnergy_Faze -- pocty pulzu do odesilanych dat

	-- zpracovani vykonu k odeslani
	if Power_Faze >= 0 then -- zaporne hodnoty nepredavam zamerne
		Rdat[Rpref.."p"] = Power_Faze -- hodnotu pridam do odesilanych dat
		if Debug == 1 then print("S> flow="..Power_Faze) end
	end
	...
	-- debug vypisy 
	if Debug == 1 and SentEnergy_Faze > 0 then
		print("S> pulse="..SentEnergy_Faze)
		end
